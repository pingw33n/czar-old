...
impl PassImpl<'_> {
    ...
    pub fn dump_base_type(&self, ty: BaseTypeId) -> String {
        ...
        write!(s, "{:12} {:32} ", self.base_type_id_str(ty.id()), self.node_kind_str(ty.node())).unwrap();
        match &ty.data {
            BaseTypeData::Struct(BaseStructType { name, params, ty }) => {
                write!(s, "Struct(name: {}, params: {}, ty: {})",
                    name,
                    self.type_list_str(params),
                    self.struct_type_str(ty),
                ).unwrap();
            }
        }
        s
    ...
    pub fn dump_type(&self, ty: TypeId) -> String {
        ...
        match &ty.data {
            &TypeData::Base(bty) => {
                write!(s, "Base({})", self.base_type_id_str(bty)).unwrap();
            }
            ...
            TypeData::Var => {
                let var = if let Some(TypeParam { name }) = self.hir(ty.node.0).try_type_param(ty.node.1) {
                    format!("{}{}", name.value.to_string(), ty.id.1.0)
                } else {
                    format!("?{}", ty.id.1.0)
                };
    ...
    fn type_id_str(&self, id: TypeId) -> String {
        let name = if id.0 == self.package_id {
            self.package_name
        } else {
    ...
    fn struct_type_str(&self, v: &StructType) -> String {
        let StructType { fields } = v;
        format!("Struct(fields: {})", self.struct_fields_str(fields))
    }
}